    ...
    public partial class Form1 : Form
    {
        ...
        private void move_button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Figure figure = (Figure)Init.figuresListBox.SelectedItem;
                    if ((figure.ToString() == "FiguresLib.Polygon") || (figure.ToString() == "FiguresLib.Triangle"))
                    {
        ...
        private void btnCar_Click(object sender, EventArgs e)
        {
            try
            {
                int x = int.Parse(car_x.Text);
                int y = int.Parse(car_y.Text);
                if (Init.Coords_check(x - 100, y - 60, 200, 153))
                {
                    car_count += 1;
                    Car car = new Car(car_count, x, y, 200, 153);
                    ShapeContainer.AddFigure(car);
                    Init.bs.ResetBindings(false);
                    car.Draw();
                }
                else
